try:
    ...
    try:
        try:
            ...
            print("Error: Could not import 'modules.errors'")
    except ImportError:
        print("Error: Could not import 'modules.errors'")
...
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'base_api_directory')))
from base_api import Core, Quality, threaded, default, FFMPEG, Callback, setup_api, UploadTime, Length, SearchingQuality, Mode
...
REGEX_VIDEO_CHECK = re.compile(r'https?://(?:www\.)?xnxx\.com/video-\w+')
REGEX_VIDEO_COMMENT_COUNT = re.compile(r'"commentCount":\s*"(\d+)"')
REGEX_VIDEO_UPLOADER = re.compile(r'"uploader":\s*"(.*?)"')
REGEX_VIDEO_M3U8 = re.compile(r'"(?:file|src)":\s*"(https?://[^"]+\.m3u8)"')
REGEX_VIDEO_TITLE = re.compile(r'"title":\s*"(.*?)"')
REGEX_VIDEO_LIKES = re.compile(r'"likes":\s*"(\d+)"')
REGEX_VIDEO_DISLIKES = re.compile(r'"dislikes":\s*"(\d+)"')
REGEX_VIDEO_KEYWORDS = re.compile(r'"keywords":\s*\["(.*?)"\]')
REGEX_SEARCH_TOTAL_PAGES = re.compile(r'"total_pages":\s*"(\d+)"')
REGEX_SCRAPE_VIDEOS = re.compile(r'video-(\w+)')
REGEX_MODEL_TOTAL_VIDEO_VIEWS = re.compile(r'"total_video_views":\s*"(\d+)"')
...
class Video:
    def __init__(self, url):
        ...
        if not REGEX_VIDEO_CHECK.search(self.url):
            raise ValueError("The video URL is invalid!")
...
def get_base_html(self):
    print(f"Attempting to fetch HTML content from {self.url}")
    content = Core().get_content(url=self.url, headers=HEADERS, cookies=None)
    if content is None:
     raise Exception(f"Failed to retrieve content from {self.url}")
    print(f"HTML content fetched: {content[:500]}")  # Print first 500 characters for debugging
    self.html_content = content.decode("utf-8")
    ...
    def get_script_content(self):
        ...
        if target_script:
            ...
            raise Exception("Couldn't extract JSON from HTML")
...
class Client:
    ...
    def get_user(cls, url):
        """
        Fetches a User object based on the provided URL.

        Args:
            url (str): The user URL.

        Returns:
            User: The User object corresponding to the provided URL.
        """
        return User(url)
